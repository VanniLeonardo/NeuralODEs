...
class VanillaGRUEncoder(nn.Module):
    ...
    def __init__(self, input_dim: int, nhidden: int, latent_dim: int):
        super().__init__()
        # Input is [x_i, m_i] only — dimension is 2 * input_dim
        self.gru = nn.GRU(input_dim * 2, nhidden, batch_first=True)
        self.h_to_mu = nn.Linear(nhidden, latent_dim)
        self.h_to_logvar = nn.Linear(nhidden, latent_dim)

    def forward(self, x: Tensor, t: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor, int]:
        # Intentionally ignore t: this encoder treats the sequence as regularly spaced.
...
class LatentODE(nn.Module):
    ...
    def __init__(self, config: LatentODEConfig):
        ...
        if encoder_type is None:
            # Back-compat: fall back to the legacy boolean use_ode_rnn.
    ...
    def _solve_latent_batched(
        ...
        # Build the pooled (union) time grid: sorted unique times across the batch.
        # We also need to know, for each (b, t) in the original grids, where it landed.
        flat_times = t_query_batch.reshape(-1)                      # (B*T,)
        pooled_times, inverse = torch.unique(
            flat_times, sorted=True, return_inverse=True
        )                                                           # pooled: (P,), inverse: (B*T,)

        # odeint expects the first time to be where z0 is defined.
        # All our grids start at 0, so pooled_times[0] == 0 and z0 is valid there.
        # If your grids ever don't start at 0, prepend t0 and handle indexing accordingly.
        z_pooled = odeint(
            self.latent_ode_func,
            z0_batch,                        # (B, latent_dim) — batched initial condition
            pooled_times,                    # (P,)
        ...
        # Re-index to recover each sample's trajectory at its own query times.
        # inverse[b*T + i] tells us which pooled index the (b, i) query time landed at.
        inverse = inverse.view(B, T)         # (B, T)
        # Gather along the pooled-time dimension for each batch element:
        # z_pooled[pooled_idx, b, :]  ->  result[b, i, :]
        batch_idx = torch.arange(B, device=z0_batch.device).unsqueeze(1).expand(B, T)
        pred_latent = z_pooled[inverse, batch_idx, :]   # (B, T, latent_dim)
    ...
    def forward(
        ...
        # --- Encoder: still per-sample (irregular grids, sequential RNN) ---
        mu_list, logvar_list = [], []
        total_encoder_nfe = 0
        ...
        mu_tensor = torch.stack(mu_list, dim=0)          # (B, latent_dim)
        logvar_tensor = torch.stack(logvar_list, dim=0)  # (B, latent_dim)

        # --- z0 sampling (batched) ---
        z0 = self.reparameterize(mu_tensor, logvar_tensor) if sample_latent else mu_tensor

        # --- Latent ODE: single batched odeint call on the pooled time grid ---
        z_t, latent_nfe = self._solve_latent_batched(z0, t_query)   # (B, T, latent_dim)

        # --- Decoder: fully batched, already ---
        pred_tensor = self.decoder(z_t)                  # (B, T, input_dim)
